...
@objc public protocol CronometroViewDelegate: class {
...
public class CronometroView: UIView {
    ...
    public func start(beginingValue: Int, interval: TimeInterval = 1) {
        ...
        timer?.invalidate()
        timer = Timer(timeInterval: fireInterval, target: self, selector: #selector(CronometroView.timerFired(_:)), userInfo: nil, repeats: true)
